...
var Printer = new function() {
	this.printLayerId = null;

	this.setPrintState = function (state) {
		localStorage.setItem("printState", state);

		if (state) {
			if (window.layer) {
				this.printLayerId = layer.open({
						type: 2
						, content: '打印中...'
					}
				);
			...
			if (window.layer) {
				layer.close(this.printLayerId);
			}
		}
	};

	this.getPrintState = function () {
		var state = localStorage.getItem("printState");
		return state;
	};
	 ...
	 * @param title 标题
	 * @param body 内容
	 * @param icon 打印的图片BASE64 默认或为空时不打印（V0无效，V1有效)
	 * @param pageNum 打印的联数 默认打印1联
	 * @constructor
	 */
	this.printTicket = function (title, body, icon) {
		if (this.getPrintState() == "true") {
			setTimeout(
				function () {
					Printer.printTicket(title, body, icon)
				}, 16000
		...
		//高版本时
		if ($$.versionCompare(plus.runtime.version, '1.1.0')) {
			printTicketV1(title, body, icon);
		} else {//低版本
			printTicketV1(title, body, icon);
		}
	};
	 ...
	 * @param title 标题
	 * @param body 内容
	 * @param icon 打印的图片BASE64 默认或为空时不打印
	 * @param pageNum 打印的联数 默认打印1联
	 */
	var printTicketV0 =
		function (title, body) {
	 ...
	 * @constructor
	 */
	var printTicketV1 = function (title, body, iconData, pageNum) {
		var BaseData = plus.android.importClass("com.allinpay.usdk.core.data.BaseData");
		var RequestData = plus.android.importClass("com.allinpay.usdk.core.data.RequestData");
		var ResponseData = plus.android.importClass("com.allinpay.usdk.core.data.ResponseData");
		...
		requestData.putValue("BUSINESS_ID", "900300003");
		requestData.putValue("PRINT_VERSION", "1");
		if (pageNum) {
			requestData.putValue("PRINT_APPEND_PAGE", pageNum + '');
		}

		var lists = new ArrayList();
		// Title
		if (title) {
			var titleStyle = new RequestData();
			titleStyle.putValue(BaseData.PRINT_APPEND_BOLD, "true");
			titleStyle.putValue(BaseData.PRINT_APPEND_SIZE, "36");
			titleStyle.putValue(BaseData.PRINT_APPEND_LOCATE, "3");
			titleStyle.putValue(BaseData.PRINT_APPEND_TEXT, title);
			lists.add(titleStyle);
		}

		// Body
		if (body) {
			var bodyStyle = new RequestData();
			bodyStyle.putValue(BaseData.PRINT_APPEND_BOLD, "false");
			bodyStyle.putValue(BaseData.PRINT_APPEND_SIZE, "24");
			bodyStyle.putValue(BaseData.PRINT_APPEND_LOCATE, "0");
			bodyStyle.putValue(BaseData.PRINT_APPEND_TEXT, body);
			lists.add(bodyStyle);
		}
		...
		if (title) {
			...
			titleStyle.putValue(BaseData.PRINT_APPEND_SIZE, "36");
			titleStyle.putValue(BaseData.PRINT_APPEND_LOCATE, "3");
			titleStyle.putValue(BaseData.PRINT_APPEND_TEXT, "\n\n\n\n" + title);
			lists.add(titleStyle);
		}
		...
		// icon
		if (iconData) {
			var QRFactory = plus.android.importClass("com.zyjzbd.pos.util.QRFactory");
			var icon=QRFactory.getBitmapBase64String(iconData);

			var contentStyle = new RequestData();
			contentStyle.putValue(BaseData.PRINT_APPEND_LOCATE, "3");
			contentStyle.putValue(BaseData.PRINT_APPEND_PIC, icon);
			contentStyle.putValue(BaseData.PRINT_APPEND_TEXT, "\n微信扫码\n领取发票");
		...
		var requsetCode = parseInt(Math.random() * 1000 + 1);
		...
		main.onActivityResult = function (requestCode, resultCode, data) {
			...
			if (1 == resultCode) {
				...
				var response = extras.getSerializable(ResponseData.KEY_ERTRAS);
				console.log(response.getValue(BaseData.REJCODE_CN));
			} else {

			}
		};

		main.startActivityForResult(intent, requsetCode);
	};
}
